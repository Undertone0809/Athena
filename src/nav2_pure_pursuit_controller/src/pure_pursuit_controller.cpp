...
void PurePursuitController::deactivate() {
  RCLCPP_INFO(logger_,
              "Dectivating controller: %s of type "
              "pure_pursuit_controller::PurePursuitController",
              plugin_name_.c_str());
...
geometry_msgs::msg::TwistStamped PurePursuitController::computeVelocityCommands(
  ...
  cmd_vel.twist.angular.z = max(-1.0 * abs(max_angular_vel_),
...
nav_msgs::msg::Path PurePursuitController::transformGlobalPlan(
  ...
  if (!transformPose(tf_, global_plan_.header.frame_id, pose, robot_pose,
  ...
  auto transformGlobalPoseToLocal = [&](const auto &global_plan_pose) {
    // We took a copy of the pose, let's lookup the transform at the current
    // time
    geometry_msgs::msg::PoseStamped stamped_pose, transformed_pose;
    stamped_pose.header.frame_id = global_plan_.header.frame_id;
    stamped_pose.header.stamp = pose.header.stamp;
    stamped_pose.pose = global_plan_pose.pose;
    transformPose(tf_, costmap_ros_->getBaseFrameID(), stamped_pose,
...
library
/home/mouse/code/github/d2lros2-humble-code/bot_ws/install/nav2_pure_pursuit_controller/lib/libnav2_pure_pursuit_controller.so.
